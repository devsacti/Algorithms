...
void bubbleup(int h[], int key, int i)
{
    ...
    int temp;
    //키값보다 부모가 크면 키값을 위로 올려야하니까 아래처럼 부등호, 한편 애당초 heap array의 인덱스가 0, 즉 절대값이 1이면 패스.
    while(i != 0 && h[p] > key)
    {
        temp = h[i];// 짐을 비우고
        ...
        // 부모노드를 가리키던 p를 child의 i에 덮어씌우면, 여기 행까진 child의 포인터도 부모노드를 가리킨다고 이해가능.
        i = p;
        //그리고 아랭세서 부모노드를 가리키는 일종의 포인터를 한층 올려서 재귀적으로 실행.
        if( i%2 == 1)
        {
            p= (i-1)/2;
        }
...
//insert, key comes from input array S by 'for statement'
void insert(int h[], int key){
    bubbleup(h, key, h_index_size);
}

//'i'를 부모노드 인데스라고 생각할 때 왼쪽, 오른쪽 자식 중에서 minimum_child's index를 찾자.
